...
// Fallback mock — only used when API returns nothing
...
function ScheduleModal({
  ...
  const handleSave = () => {
    if (!date) return;
    onSave({ courseId: course.id, date, time, note });
    setDone(true);
  };
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="date" value={date} onChange={(e) => setDate(e.target.value)} min={new Date().toISOString().slice(0, 10)}
                  className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-slate-800 outline-none focus:border-blue-400" />
              </div>
              <div>
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute right-10 top-4 rounded-lg p-1.5 hover:bg-yellow-50 transition"
      ...
      <button
        ...
        className="absolute right-4 top-4 rounded-lg p-1.5 hover:bg-blue-50 transition"
        title="Schedule"
      >
...
export default function StudentCoursesHub() {
  ...
  // Load saved/schedules from localStorage (per-browser session)
  useEffect(() => {
    ...
    fetch("/api/student/courses")
      ...
      .then((data) => {
        if (data.courses && data.courses.length > 0) {
          const shaped: Course[] = data.courses.map((c: any) => ({
            id: c.id,
            title: c.title,
            ...
            status: c.enrollment?.enrollmentStatus === "COMPLETED"
              ? "completed"
              : c.enrollment
              ? "in_progress"
              : "not_started",
    ...
    <div className="space-y-6 p-5">
      ...
      <div className="flex gap-2 overflow-x-auto">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setActiveTab(id)}
            ...
            {id === "saved" && savedIds.size > 0 && (
              <span className="ml-1 rounded-full bg-yellow-400 px-1.5 py-0.5 text-[10px] font-black text-white">{savedIds.size}</span>
            )}
            {id === "schedule" && schedules.length > 0 && (
              <span className="ml-1 rounded-full bg-blue-400 px-1.5 py-0.5 text-[10px] font-black text-white">{schedules.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="relative">
        ...
        <input type="search" value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-9 pr-4 text-sm text-slate-800 outline-none focus:border-blue-400" />
      </div>
              ...
              .map((s) => {
                ...
                return (
                  <div key={s.courseId} className="flex items-center justify-between px-5 py-3">
                    ...
                    <button onClick={() => removeSchedule(s.courseId)}
                      className="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-400 transition">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-blue-100 bg-white/60 py-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            {activeTab === "saved" ? "No saved courses yet. Star a course to save it." :
             activeTab === "schedule" ? "No scheduled sessions. Click the calendar icon on a course." :
             "No courses found."}
          </p>
        </div>
      ) : (

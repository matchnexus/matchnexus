...
import { HiPlus, HiTrash, HiX, HiAcademicCap, HiCheckCircle, HiArrowLeft } from "react-icons/hi";
...
const levelColor: Record<CourseLevel, string> = { BEGINNER: "success", INTERMEDIATE: "info", ADVANCED: "warning" };
const statusColor: Record<CourseStatus, string> = { PUBLISHED: "success", DRAFT: "gray", ARCHIVED: "failure" };

function CreateCourseModal({ onClose, onCreate }: { onClose: () => void; onCreate: (c: Course) => void }) {
  const [form, setForm] = useState({ title: "", description: "", level: "BEGINNER" as CourseLevel, duration: "", priceAmount: "", isFree: false, status: "DRAFT" as CourseStatus });
          ...
          <div className="p-6 space-y-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.isFree} onChange={(e) => set("isFree", e.target.checked)} className="rounded border-gray-300 text-blue-600" />
                <span className="text-sm font-semibold text-gray-700">Free course</span>
              </label>
              <div className="flex justify-end gap-3 pt-2 border-t">
                <button type="button" onClick={onClose} className="rounded-xl border border-gray-200 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-gray-600 hover:bg-gray-50 transition">Cancel</button>
                <button type="submit" className="rounded-xl bg-gray-900 px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-blue-600 transition">Create Course</button>
              </div>
            </form>
          </div>
        )}
...
function DeleteConfirmModal({ course, onClose, onConfirm }: { course: Course; onClose: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl space-y-4">
        <div className="flex items-center gap-3">
          <div className="rounded-xl bg-red-100 p-2.5"><HiTrash className="h-5 w-5 text-red-600" /></div>
          <h3 className="text-base font-bold text-gray-900">Delete Course</h3>
        </div>
        <p className="text-sm text-gray-600">Are you sure you want to delete <span className="font-semibold">"{course.title}"</span>? This cannot be undone.</p>
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button onClick={onConfirm} className="rounded-xl bg-red-600 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-red-700 transition">Delete</button>
        </div>
      </div>
    </div>
  );
...
export default function AdminCoursesStandalonePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {showCreate && <CreateCourseModal onClose={() => setShowCreate(false)} onCreate={(c) => { handleCreate(c); }} />}
      {deleteTarget && <DeleteConfirmModal course={deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} />}

      {/* Navbar */}
      <header className="sticky top-0 z-30 border-b border-gray-200 bg-white shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            <Link href="/" className="group flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-slate-500 hover:text-blue-600 transition">
              <HiArrowLeft className="transition-transform group-hover:-translate-x-1" /> Back
            </Link>
            <span className="h-5 w-px bg-slate-200" />
            <span className="text-lg font-extrabold tracking-tight text-gray-900">MatchNexus <span className="text-blue-600">Admin</span></span>
          </div>
          <button onClick={() => setShowCreate(true)}
            className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-blue-600 transition">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8 space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Create and manage courses available to students.</p>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {courses.length === 0 ? (
                <tr><td colSpan={7} className="py-10 text-center text-sm text-gray-400">No courses yet. Create one to get started.</td></tr>
              ) : (
                courses.map((course) => (
                  <tr key={course.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="py-4 pr-6">
                      <Badge color={levelColor[course.level] as any} className="w-fit px-2 py-1 text-[10px] font-bold uppercase">{course.level}</Badge>
                    </td>
                    ...
                    <td className="py-4 pr-6">
                      <Badge color={statusColor[course.status] as any} className="w-fit px-2 py-1 text-[10px] font-bold uppercase">{course.status}</Badge>
                    </td>
